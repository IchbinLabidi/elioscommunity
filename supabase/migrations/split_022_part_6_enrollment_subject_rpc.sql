...
begin
  ...
  insert into public.course_enrollments(course_id, subject_id, student_id, teacher_id, status, payment_proof_url, payment_proof_path, payment_note)
  values (course_row.id, course_row.subject_id, auth.uid(), course_row.teacher_id, 'pending', proof_url, proof_path, nullif(btrim(coalesce(payment_note, '')), ''))
  ...
  do update set
    ...
    reviewed_by = null,
    reviewed_at = null
  where public.course_enrollments.status in ('pending','rejected')
